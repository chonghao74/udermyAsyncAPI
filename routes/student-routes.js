...
router.get("/number/:numberData", cors(), async (req, res) => {
  let { numberData } = req.params;

  try {
    dbData = await Student.find({ number: numberData })
      .exec()
      .then((d) => {
        console.log(d);
        return {
...
router.get("/name/:nameData", async (req, res) => {
  ...
  try {
    dbData = await Student.find({ name: nameData })
      .exec()
      .then((d) => {
        console.log(d);
        return {
...
router.get("/name", async (req, res) => {
  ...
  try {
    dbData = await Student.find({ name: name })
      .exec()
      .then((d) => {
        return {
          status: 200,
...
function deleteOneDataByName(res, name) {
  try {
    Student.deleteOne({ name: name }).then((d) => {
      dbData = {
        status: 200,
        data: d,
      };
...
router.put("/", async (req, res) => {
  ...
  try {
    dbData = await Student.findOneAndUpdate(filter, update, {
      new: true,
      runValidators: true,
      overwrite: true,
    })
...
router.patch("/:_id", async (req, res) => {
  ...
  if (!req.params._id.trim()) {
    ...
    try {
      ...
      console.log(
        "(findOneAndUpdate)Patch endPoint /api/students :property Error"
      );
      dbData = {
        status: 6999,
        data: e,
